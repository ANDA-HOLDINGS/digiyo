...
const FullVideoScreen = ({ navigation, route }) => {
  ...
  const currentIndex = postsArray.findIndex(
    (postItem) => postItem.post_id === item.post_id
  );
  ...
  const onViewableItemsChanged = useCallback(({ viewableItems }) => {
    const visibleVideoIndices = viewableItems
      .filter((item) => item.item.media_items[0].type === "video")
      .map((item) => item.index);

    const newVisibleVideos = postsArray.map((_, index) =>
      visibleVideoIndices.includes(index)
    );
              ...
              return (
                <View style={{}}>
                  ...
                  <FullScreenLikeIcons
                    ...
                    openMenuBottomSheetHandler={() =>
                      setOpenMenuBottomSheet(true)
                    }
                  />
                  <CommentsBottomSheet
